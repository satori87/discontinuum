...
public class IntFloatMap implements Iterable<Entry>
{
    ...
    public IntFloatMap(final int initialCapacity) {
        this(initialCapacity, 0.8f);
    }
    
    public IntFloatMap(int initialCapacity, final float loadFactor) {
    ...
    public IntFloatMap(final IntFloatMap map) {
    ...
    public void putAll(final IntFloatMap map) {
        for (final Entry entry : map.entries()) {
            this.put(entry.key, entry.value);
        }
    }
    
    private void putResize(final int key, final float value) {
        if (key == 0) {
            this.zeroValue = value;
            this.hasZeroValue = true;
            return;
    ...
    private void putStash(final int key, final float value) {
    ...
    private float getStash(final int key, final float defaultValue) {
        final int[] keyTable = this.keyTable;
        for (int i = this.capacity, n = i + this.stashSize; i < n; ++i) {
            if (key == keyTable[i]) {
    ...
    public float getAndIncrement(final int key, final float defaultValue, final float increment) {
        if (key != 0) {
            int index = key & this.mask;
        ...
        if (this.hasZeroValue) {
            final float value2 = this.zeroValue;
    ...
    private float getAndIncrementStash(final int key, final float defaultValue, final float increment) {
        final int[] keyTable = this.keyTable;
        for (int i = this.capacity, n = i + this.stashSize; i < n; ++i) {
            if (key == keyTable[i]) {
    ...
    public float remove(final int key, final float defaultValue) {
        if (key == 0) {
            if (!this.hasZeroValue) {
                return defaultValue;
            }
    ...
    float removeStash(final int key, final float defaultValue) {
        final int[] keyTable = this.keyTable;
        for (int i = this.capacity, n = i + this.stashSize; i < n; ++i) {
            if (key == keyTable[i]) {
    ...
    public boolean containsValue(final float value, final float epsilon) {
    ...
    public int findKey(final float value, final int notFound) {
        if (this.hasZeroValue && this.zeroValue == value) {
            return 0;
        }
        final int[] keyTable = this.keyTable;
    ...
    public boolean equals(final Object obj) {
        ...
        final IntFloatMap other = (IntFloatMap)obj;
    ...
    public static class Entries extends MapIterator implements Iterable<Entry>, Iterator<Entry>
    {
        ...
        public Entries(final IntFloatMap map) {
            super(map);
    ...
    {
        public Keys(final IntFloatMap map) {
            super(map);
        }
    ...
    {
        public Values(final IntFloatMap map) {
            super(map);
        }
    ...
    private static class MapIterator
    {
        ...
        public MapIterator(final IntFloatMap map) {
            this.valid = true;
            this.map = map;
            this.reset();
        }
        ...
        public void remove() {
            ...
            this.currentIndex = -2;
            final IntFloatMap map = this.map;
            --map.size;
        }
    }
}
